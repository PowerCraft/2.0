 ...
 * использовании не происходит загрузки классов. Однако, сама по себе загрузка
 * классов редко является проблемой, потому что инициализация класса (вызов
 * статических блоков) происходит не при загрузке класса. Проблемы появляются,
 * когда хуки вставляются в зависимые друг от друга классы, тогда стандартная
 * реализация отваливается с ClassCircularityError.
 */
