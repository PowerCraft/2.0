...
import powercraft.api.hooklib.asm.AsmHook;
import powercraft.api.hooklib.asm.HookClassTransformer;
import powercraft.api.hooklib.asm.HookInjectorClassVisitor;
...
public class PrimaryClassTransformer extends HookClassTransformer implements IClassTransformer {
	...
	// костыль для случая, когда другой мод дергает хуклиб раньше, чем она
	// запустилась
	...
	protected HookInjectorClassVisitor createInjectorClassVisitor(ClassWriter cw, List<AsmHook> hooks) {
		// Если ничего не сломается, то никакие майновские классы не должны грузиться
		// этим трансформером -
